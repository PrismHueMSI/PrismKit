...
public final class PrismDeviceKeyboard: PrismDevice {
    ...
    public static var keys = [PrismKey]()

    // Selected keys array

    public static var keysSelected = [PrismKey]()

    // Effects array

    public static var effects = [PrismEffect]()

    // Origin

    public static let origin = PrismPoint()
    ...
    deinit {
        PrismDeviceKeyboard.keys.removeAll()
        PrismDeviceKeyboard.keysSelected.removeAll()
        PrismDeviceKeyboard.effects.removeAll()
    }
}
...
extension PrismDeviceKeyboard {
    ...
    private func writeEffectsToKeyboard() -> IOReturn {
        let effects = PrismDeviceKeyboard.effects
    ...
    private func updatePerKeyKeyboard(forceUpdate: Bool = false) {
        commandMutex.async {
            let keysSelected = PrismDeviceKeyboard.keysSelected
...
//                .compactMap { $0.prismKey }
    ...
    private func writeKeysToKeyboard(region: UInt8, keycodes: [UInt8]) -> IOReturn {
        ...
        let keyboardKeys = PrismDeviceKeyboard.keys.filter { $0.region == region }
    ...
    public static func getRegionFromKeycode(_ keycode: UInt8) -> UInt8 {
